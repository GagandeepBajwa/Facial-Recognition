...
public class CNN {
    ...
    private static ComputationGraph nn_init(double learningRate) {
        ...
        ComputationGraphConfiguration config = new NeuralNetConfiguration.Builder().weightInit(WeightInit.XAVIER)
                ...
                .addLayer("INPUT_I1", new ConvolutionLayer.Builder() // First layer of type Convolutional, we name it
                                                                     // INPUT_I1
                        .kernelSize(3, 3) // Default receptive field of 2,2
                        .stride(3, 3).nIn(INPUT_CHANNELS) // 3 input channels (red,green,blue) to start
                        .nOut(CONV_2_NODES) // We want 8 feature maps
                        .build(), "vector_in")

                .addLayer("SUB1", new SubsamplingLayer.Builder(PoolingType.MAX).kernelSize(3, 3) // Standard pool kernel
                                                                                                 // of 2x2. Keep the
                                                                                                 // largest value (max
                                                                                                 // pooling).
                ...
                .addLayer("CONV2",
                        new ConvolutionLayer.Builder().kernelSize(1, 1).stride(1, 1).nIn(CONV_2_NODES)
                                .nOut(CONV_2_OUTPUTS).build(),
                        "SUB1")

                .addLayer("SUB2", new SubsamplingLayer.Builder(PoolingType.MAX).kernelSize(3, 3).stride(3, 3).build(),
                        "CONV2")

                /**
                 * Here is where we transition from convolutional to standard feed forward
                 * network. Note that for this transition you don't need to specify an nIn().
                 * DeepLearning4j will figure this out for you to automatically connect the
                 * convolution part of the network to the feed forward
                 */

                .addLayer("DENSE1", new DenseLayer.Builder() /** Standard feed forward from here */
                        .nOut(HIDDEN_H2_NODES).build(), "SUB2")

                .addLayer("HIDDEN_H2", new DenseLayer.Builder().nIn(HIDDEN_H2_NODES).nOut(HIDDEN_H3_NODES).build(),
                        "DENSE1")

                .addLayer("HIDDEN_H3", new DenseLayer.Builder().nIn(HIDDEN_H3_NODES).nOut(HIDDEN_H4_NODES).build(),
                        "HIDDEN_H2")

                .addLayer("HIDDEN_H4", new DenseLayer.Builder().nIn(HIDDEN_H4_NODES).nOut(HIDDEN_H5_NODES).build(),
                        "HIDDEN_H3")

                .addLayer("HIDDEN_H5", new DenseLayer.Builder().nIn(HIDDEN_H5_NODES).nOut(OUTPUT_NODES).build(),
                        "HIDDEN_H4")

                .addLayer("OUTPUT_O1",
                        new OutputLayer.Builder(LossFunctions.LossFunction.MCXENT).activation(Activation.SOFTMAX)
                                .nIn(OUTPUT_NODES).nOut(OUTPUTS) // we have 10 outputs
                                .build(),
                        "HIDDEN_H5")
                ...
                /**
                 * Important: you must tell the CNN the height,width and depth(channels) of the
                 * images you are going to use. In this example the square that slides across
                 * the GUI looking for Johnny is 128x94 height x width and because we are using
                 * RGB the channels are 3.
                 */
    ...
    public static void train2(int epoch, ArrayList<INDArray> training_set, Dataset ds) throws Exception {
        ...
        for (; epoch > 0; epoch--) {
            ...
            if (epoch % 25 == 0) {
                model.save(new File("cnn" + "_" + epoch + "_" + score / counter + ".zip"), true);
            }
        }
    ...
    public static void test(Dataset ds) throws Exception {
        ...
        /**
         * This demo takes a lot of memory in forcing some kind of garbage collection
         * seems to help (eliminated out-of-memory errors)
         */
